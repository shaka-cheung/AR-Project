...
void initGame() {
    // team blue (id 5, 10, 80, 160)
    figures[5] = new Figure(BLUE, true, false);
    figures[10] = new Figure(BLUE, true, true);
    figures[80] = new Figure(BLUE, false, false);
    figures[160] = new Figure(BLUE, false, true);

    // red team (id 7, 112, 142, 232)
    figures[7] = new Figure(RED, true, false);
    figures[112] = new Figure(RED, true, true);
    figures[142] = new Figure(RED, false, false);
    figures[232] = new Figure(RED, false, true);

    // yellow team (id 9, 15, 264, 4104)
    figures[9] = new Figure(YELLOW, true, false);
    figures[15] = new Figure(YELLOW, true, true);
    figures[264] = new Figure(YELLOW, false, false);
    figures[4104] = new Figure(YELLOW, false, true);

    // green team (id 6, 70, 96, 608)
    figures[6] = new Figure(GREEN, true, false);
    figures[70] = new Figure(GREEN, true, true);
    figures[96] = new Figure(GREEN, false, false);
    figures[608] = new Figure(GREEN, false, true);
}
...
void autoCalButton(int i, void *arg) {
    autocalibrate = i;
}

void calStonesButton(int i, void *arg) {
    calstones = i;
}

void restartButton(int i, void *arg) {
    for (int i = 0; i < 4; i++)
        winner[i] = false;
}
...
void init()
{
    ...
    cvCreateButton("autocalibrate",autoCalButton,NULL,CV_CHECKBOX,1);
    cvCreateButton("calibration-stones",calStonesButton,NULL,CV_CHECKBOX,1);
    cvCreateButton("restart",restartButton);
...
//now we have a B/W image of a supposed Marker
            ...
            if (code >= 1 && code <= 4) {
                ...
                //if (cal_found_marker.find(code) == cal_found_marker.end()) {
			        ...
			        estimateSquarePose( fieldResultMatrix[code - 1], corners, 0.042);
                  
                //}
                    
                // check whether calibration can begin
                if (cal_marker_counter == 4 && !calibrated) {
                ...
                // check whether its a pawn marker
                if (figures.find(code) != figures.end()) {
                    // 1) find marker center
                    CvPoint2D32f sPosition = intersect(corners[0], corners[1], corners[2], corners[3]);                
                    figures[code]->setScreenInfo(sPosition);                    
			        ...
			        float resultMatrix[16];
				    estimateSquarePose(resultMatrix, corners, 0.042);
				    figures[code]->setMatrix(resultMatrix);
                }            
            ...
            if (calibrated) {
                ...
                for (auto val : figures) {
                    ...
                    float dist = f->getDistance(startFields[f->getTeam()]);
                    
                    // check whether user is in start position
                    if (dist < 5.0f && !f->animatedSinceLastMove()) {
                        f->invokeAnimation(Figure::START);
                    }

                    // check whether there is a beating
                    for (auto val2 : figures) {
                        Figure *f2 = val2.second;

                        // check for beating
                        if (f->getTeam() != f2->getTeam()) {
                            float dist = f->getDistanceToFigure(f2);
                            
                            if (dist > 0.1f && dist < 10.0f) {
                                if (f->wasMoved() && !f->animatedSinceLastMove()) {

                                    // f2 is victim
                                    f->setLookAtMatrix(f2->getMatrix());
		                            f2->setLookAtMatrix(f->getMatrix());
		                            f->invokeAnimation(Figure::BEAT);
		                            f2->invokeAnimation(Figure::DEFEAT);

                                } else if (f2->wasMoved() && !f2->animatedSinceLastMove()) {

                                    // f is victim
                                    f->setLookAtMatrix(f2->getMatrix());
		                            f2->setLookAtMatrix(f->getMatrix());
		                            f->invokeAnimation(Figure::DEFEAT);
                                    f2->invokeAnimation(Figure::BEAT);
                    ...
                    // check whether user is in house position
                    for (int i = 0; i < 4; i++) {
                        float dist = f->getDistance(houseFields[f->getTeam()][i]);

                        // check whether user is in start position
                        if (dist < 5.0f && !f->animatedSinceLastMove()) {
                            f->invokeAnimation(Figure::WIN);
                ...
                // check whether we got a winner
                int fcounter[4] = {0, 0, 0, 0};
                for (auto val : figures) {
                    if (val.second->isInHouse()) {
                        fcounter[val.second->getTeam()]++;
                    }
                }
                for (int i = 0; i < 4; i++) {
                    if (fcounter[i] == 4 && !winner[i]) {
                        // animate team
                        for (auto val : figures) {
                            if (val.second->getTeam() == i) {
                                val.second->invokeAnimation(Figure::VIC);
                            }
                        } 
                        winner[i] = true;   
                    }
...
// Added in Exercise 8 - End *****************************************************************
    ...
    if (calibrated) {
        ...
        //glLoadTransposeMatrixf( fieldResultMatrix[0]);

        if (autocalibrate)
            calcPoints();

        for (auto val : figures) {
            val.second->calcWorldCoords();
        }

        if (calstones) {
        for (int i = 0; i < 4; i++) {   

            // Transforms every screen coordinates to world coordinates
...
// manuel calibration stuff
bool tracking = false;
enum FieldType { startField, houseField } fType;
int fTrackedTeam;
int fTrackedID;

void mouseTracker(int button, int state, int x, int y) {
    CvPoint3D32f mW = GetOGLPos(x, y);
    ...
    if (!tracking && state == 0) {
        ...
        for (int i = 0; i < 4; i++) {
            float dist = getDistanceOfPoints(mW, startFields[i]);

            if (dist < 5.0) {
                fTrackedTeam = i;
                fType = FieldType::startField;
                tracking = true;
                break;
            }
        }

        // search for house-field
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                float dist = getDistanceOfPoints(mW, houseFields[i][j]);

                if (dist < 5.0) {
                    fTrackedTeam = i;
                    fTrackedID = j;
                    fType = FieldType::houseField;
                    tracking = true;
                    break;
                }
            }
        }
    } else if (tracking && state == 1) {
        // store new position            
        if (fType == FieldType::startField) {
            cout << "set start field" << endl;
            startFields[fTrackedTeam] = mW;
            tracking = false;
        } else {
            houseFields[fTrackedTeam][fTrackedID] = mW;
            tracking = false;
        }
    }
    }
}
// end manuel calibration stuff
...
int main(int argc, char* argv[]) 
{
    ...
    glutIdleFunc( idle );
    glutTimerFunc(25, timer, 0); // animation stuff
    glutMouseFunc(mouseTracker); // manuell calibration stuff
